...
import { KNOWN_SESSION_EVENT_TYPES, SessionPreparation, canonicalHeader } from "@deepseek-ai/dsh-session";
...
function registerCodexSessionEventType() {
	if (KNOWN_SESSION_EVENT_TYPES.has(CODEX_THREAD_LINK_EVENT)) return () => {};
	const mutable = KNOWN_SESSION_EVENT_TYPES;
	if (typeof mutable.add !== "function" || typeof mutable.delete !== "function") throw new Error("agent-codex: this Harness cannot register external Session event types");
	mutable.add(CODEX_THREAD_LINK_EVENT);
	return () => {
		mutable.delete(CODEX_THREAD_LINK_EVENT);
	};
}
